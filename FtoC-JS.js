...
const weekWork = function weekWork() {
  var week = 0;
  var work = 0;
  for (let entry of JOURNAL) {
      for (let e of entry.events) {
          if (e.indexOf("weekend") > -1) {
              week++;
          }
          if (e.indexOf("work") > -1 ) {
              work++;
  ...
  console.log("weekend sayisi : ", week);
  console.log("work sayisi: ", work);
}
const mapper = new Map();
const assignMapper = function assignMap() {
  for (let entry of JOURNAL) {
      if (entry.events.includes('weekend')) {
          for (let e of entry.events) {
              if (mapper.has(e)) {
                  mapper.set(e, mapper.get(e) + 1);
              } else {
                  mapper.set(e, 1);
              }
          }
